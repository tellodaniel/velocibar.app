...
Opening Speedtest.net or Fast.com and waiting a few seconds is the fastest way to check your connection. For a quick spot check — is the wifi working right now? did the technician actually fix the line? — it's more than enough, and you don't need to install anything.
...
- **Don't trust a single reading.** Both web tests and `networkquality` can vary from one run to the next for reasons unrelated to your connection — momentary server congestion, for instance. Run the test two or three times and go with the trend, not the outlier.
